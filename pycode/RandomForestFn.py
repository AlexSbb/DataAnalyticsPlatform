...
def RFreg(RF_inputs):
    try:
        ...
        if  len(RF_inputs.X) != len(RF_inputs.y):                                           #Check size of input and output
        ...
        elif RF_inputs.test_size < minTest_size or RF_inputs.test_size > maxTest_size:      #Check test size is in range 0.1 to 0.9
            RF_outputs.flag = error
            ...
            X_train, RF_outputs.X_test, y_train, RF_outputs.y_actual = train_test_split(rfX2, rfy1, test_size= RF_inputs.test_size, random_state=defSplit)
